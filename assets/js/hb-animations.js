...
document.addEventListener("alpine:init", () => {
  ...
  Alpine.data("typewriter", (strings = [], options = {}) => ({
    ...
    randomIndex() {
      if (this.strings.length <= 1) return 0;

      let nextIndex = this.currentIndex;
      while (nextIndex === this.currentIndex || (this.strings.length > 2 && nextIndex === this.previousIndex)) {
        nextIndex = Math.floor(Math.random() * this.strings.length);
      }
    ...
    type() {
      ...
      if (!this.isDeleting && this.currentText === current) {
        ...
        // Finished deleting, randomly pick another string.
        this.isDeleting = false;
        this.previousIndex = this.currentIndex;
        this.currentIndex = this.randomIndex();

        if (!this.loop && this.previousIndex >= 0) {
          this.currentText = this.strings[this.previousIndex];
          return;
        }
